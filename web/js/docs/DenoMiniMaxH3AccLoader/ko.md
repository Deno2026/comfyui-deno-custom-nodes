...
FL2VA/T2VA에는 FL2VA Acc-LoRA를, Ref2VA에는 Ref2VA Acc-LoRA를 사용하세요. 공식 학습·권장 설정은 여전히 Simple/Euler 8-step입니다. 9-step, 10-step, 다른 내림차순 스케줄, 레이턴트 업스케일용 분할 sigma 패스도 실험할 수 있지만 화질 향상을 보장하지는 않습니다. strength는 `1.0`, 영상/오디오 sigma shift는 순정 값인 `12.0 / 3.0`을 유지하세요. 곡선 압축된 pruned 모델에서는 직접 적용할 수 없는 full-width AdaLN LoRA 50개만 호환 모드에서 건너뛰고 나머지 LoRA와 PDD 헤드는 모두 적용합니다. 완전판 모델에서는 전체 어댑터를 적용합니다.
